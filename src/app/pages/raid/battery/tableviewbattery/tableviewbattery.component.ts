...
import { dashboard, Pingreport, RAIDModel } from '../../../../models/fullmodels';
...
@Component({
  selector: 'app-tableviewbattery',
  templateUrl: './tableviewbattery.component.html',
  styleUrl: './tableviewbattery.component.css'
})
export class TableviewbatteryComponent {
  fileName: string = 'Raidbattery_Reports_'+new Date().toISOString().replace(/\D/g, '').replace(/-/g, '');
...
var processedcolumns = columnKeys.map(key => {
  ...
  LoadData() {
    this.ApiService.getDataRaidBatteryStatus().subscribe((data) => {
