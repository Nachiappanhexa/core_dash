...
@Component({
  selector: 'app-tableviewserverbackup',
  templateUrl: './tableviewserverbackup.component.html',
  styleUrl: './tableviewserverbackup.component.css'
})
export class TableviewserverbackupComponent {
  @Input() data: BackupServer[] = [];
  fileName: string = 'Serverbackup_Reports_' + new Date().toISOString().replace(/\D/g, '').replace(/-/g, '');
...
var processedcolumns = columnKeys.map(key => {
  ...
  LoadData() {
      ...
      this.columns = Object.keys(this.data).map(key => {
        return {
          prop: key,
