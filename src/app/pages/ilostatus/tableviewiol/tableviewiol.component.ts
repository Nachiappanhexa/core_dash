...
@Component({
  selector: 'app-tableviewiol',
  templateUrl: './tableviewiol.component.html',
  styleUrl: './tableviewiol.component.css'
})
export class TableviewiolComponent {
  @Input() data: ILOreport[] = [];
  fileName: string = 'IOL_Reports_' + new Date().toISOString().replace(/\D/g, '').replace(/-/g, '');
  ...
  var processedcolumns = columnKeys.map(key => {
    ...
    @ViewChild('Mytableiol', {static: false}) Mytable!: ElementRef;
